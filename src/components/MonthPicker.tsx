...
export default function MonthPicker({ value, onChange }: DatePickerProps) {
  ...
  const selectedYear = parts.length >= 1 ? parseInt(parts[0]) : new Date().getFullYear();
  const selectedMonth = parts.length >= 2 ? parseInt(parts[1]) : new Date().getMonth() + 1;
  ...
  const handleOpen = () => {
    ...
    setTempDay(selectedDay && selectedYear === viewYear && selectedMonth === viewMonth ? selectedDay : selectedDay);
    setOpen(true);
  };
            ...
            <div className="flex items-center justify-between px-6 mb-4 shrink-0">
              ...
              <div className="flex gap-2">
                <button type="button" onClick={prevMonth}
                  className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-toss-gray-100 transition text-toss-blue text-[16px] font-bold">
                  ‹
                </button>

...
function statusBadge(status: string) {
  switch (status) {
    ...
    case "processing":
      return { label: "처리 중", bg: "bg-blue-50", text: "text-toss-blue" };
    default:
      return { label: "작업 전", bg: "bg-toss-gray-50", text: "text-toss-gray-500" };
  }
}

export default function YouTubeShortsDashboard() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (!isAdmin && !w.allowedServices?.includes("youtube-shorts")) {
        router.push("/");
        return;
      }
      setWorker(w);
  ...
  const handleNewProject = async () => {
    ...
    setCreating(true);
    try {
      const resp = await fetch("/api/youtube-shorts/projects", {
        method: "POST",
      ...
      if (data.id) {
        router.push(`/youtube-shorts/${data.id}`);
      }
    } catch {
  ...
  const handleDelete = async (e: React.MouseEvent, id: string) => {
    ...
    await fetch(`/api/youtube-shorts/projects/${id}`, { method: "DELETE" });
  ...
  const handleBulkDelete = async () => {
    ...
    await Promise.all(
      [...selected].map((id) =>
        fetch(`/api/youtube-shorts/projects/${id}`, { method: "DELETE" })
      )
    );
      ...
      <div className="max-w-3xl mx-auto px-5 mt-6">
        ...
        {!loading && projects.length === 0 && (
          <div className="text-center py-24">
            ...
            <p className="text-sm text-toss-gray-400 mb-6">첫 번째 프로젝트를 만들어 쇼츠 제작을 시작해 보세요</p>
            <button
              onClick={handleNewProject}
              disabled={creating}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {projects.map((p) => {
              ...
              return (
                <div
                  key={p.id}
                  onClick={() => selectMode ? toggleSelect(p.id) : router.push(`/youtube-shorts/${p.id}`)}

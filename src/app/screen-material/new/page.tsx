...
export default function NewScreenMaterialProject() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (!worker.isAdmin && !worker.allowedServices?.includes("screen-material")) {
        router.replace("/"); return;
      }
    } catch { router.replace("/"); return; }

    fetch("/api/screen-material/projects", {
      method: "POST",
      ...
      .then((r) => r.json())
      .then((data) => {
        if (data.id) router.replace(`/screen-material/${data.id}`);
        else router.replace("/screen-material");
      })
      .catch(() => {
        alert("프로젝트 생성에 실패했습니다.");
        router.replace("/screen-material");
      });
  }, [router]);
